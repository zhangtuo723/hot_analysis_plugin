...
6. **滚动策略**：页面内容或评论区不在视口内时，优先使用 `scroll_to_element` 定位到具体元素；需要加载更多内容时，使用 `scroll_page` 滚动特定容器或整个页面
7. **生成报告**：当用户需要保存或分享分析结果时，使用 `generate_report` 工具生成可视化 HTML 报告，返回链接供用户点击查看"""
        ...
        async for chunk in agent.astream(
            ...
            for node_name, node_output in chunk.items():
                ...
                if node_name == "model" and isinstance(node_output, dict):
                    for msg in node_output.get("messages", []):
                        ...
                        if hasattr(msg, "tool_calls") and msg.tool_calls:
                            for tc in msg.tool_calls:
                                ...
                                raw_id = tc.get("id")
                                tool_id = str(raw_id) if raw_id is not None else ""
                                if tool_id:
                    ...
                    for msg in node_output.get("messages", []):
                        # tool_call_id 是工具调用与结果的唯一配对凭证，前端按它匹配 tool_start
                        raw_call_id = getattr(msg, "tool_call_id", None)
                        tool_id = str(raw_call_id) if raw_call_id is not None else ""
                        ...
                        # fallback：若 map 中找不到，尝试 msg.name 或使用 unknown
                        if not tool_name:
